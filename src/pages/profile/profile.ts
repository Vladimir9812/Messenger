...
import { ArrowLink, Avatar, Button, Modal, ChangeAvatar } from '@components';
...
export class ProfilePage extends ConnectBlock<SuperProps> {
  ...
  constructor() {
		...
		const form = new ProfileForm<UserFormData | PasswordFormData>({
			fields: fields.map(field => ({ ...field, mode: 'horizontal' })),
			isMainState,
    ...
    const superProps: SuperProps = {
      avatar: new Avatar({
				mode: 'big',
				imgSrc: Store.getState(AuthUserAvatar),
				hover: state === 'edit',
				onClick: () => {
    ...
    super('div', 'profile', superProps, AuthUserAvatar);
    ...
    return this.compile(ProfileTemplate, {
			...
			name: Store.getState(AuthUser)?.first_name
		});
  }
}
