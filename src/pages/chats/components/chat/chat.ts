...
import { classNames, AuthUser, SelectedChatId } from '@utilities';
...
export class Chat extends Block<SuperProps> {
  ...
  constructor(props: Props) {
    ...
    const superProps: SuperProps = {
      ...
      onClick: () => {
				if (props.chat.id === Store.getState(SelectedChatId)) {
					return;
				}
				Router.go(`${CHATS_PATH}?viewId=${props.chat.id}`);
			}
    };
  ...
  render(): DocumentFragment {
    ...
    return this.compile(ChatTemplate, {
      ...
      login: user?.login,
      isYouLast: user?.login === Store.getState(AuthUser)?.login,
      message: content,
